...
if __name__ == '__main__':
    ...
    for fp in all_getfp_labels:
        ...
        for sim in all_similarity_labels:
            ...
            #reset the validation dataframe (everytime you go through this loop, you add to datasub_val- that needs to be cleared)
            datasub_val = data[data['dataset']== 'val']
            ...
            train_fp = Parallel (n_jobs=25, verbose=3)(delayed(get_fp_parallel)(smi, getfp_label) for smi in datasub['react_smiles'])
            datasub['react_fp'] = train_fp
    
            print ('Getting fingerprint information for the validation set: {}'.format (getfp_label))
            val_fp = Parallel (n_jobs=25, verbose=3)(delayed(get_fp_parallel)(smi, getfp_label) for smi in datasub_val ['react_smiles'])
            ...
            os.chdir (os.path.join(root_path, '4.TopK_Accuracy_Evaluation', 'out_val'))
            
            #save the results
            datasub_val.to_pickle ('datasub_val_result_{}_{}.pkl'.format(getfp_label,similarity_label))
            datasub_val.to_csv ('datasub_val_result_{}_{}.csv'.format(getfp_label,similarity_label))
